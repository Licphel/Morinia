...
public class LightEngineImpl : LightEngine
{
	...
	{
		int d = (int) (MaxValue / Unit / 2f);
		for(int xp = (int) x - d; xp <= d + x; xp++)
		{
			for(int yp = (int) y - d; yp <= d + y; yp++)
			{
				float dst = FloatMath.Sqrt(FloatMath.Pow(xp - x, 2) + FloatMath.Pow(yp - y, 2));
				if(dst > d) continue;
				float fac = 1 - dst / d;
				DrawDirect(xp, yp, v1 * fac, v2 * fac, v3 * fac);
			}
		}
	}
